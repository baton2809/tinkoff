...
class ReinforceTrainer:
    ...
    def _setup_config(self) -> None:
        ...
        self.reward_model_path = self.config.model_path
        ...
        for batch_idx in range(batch_size):
            try:
                if self.train_dataset is None or len(self.train_dataset) == 0:
                    print("Тренировочный датасет недоступен, пропускаем..")
                    continue
    ...
    def get_reward(self, text: str) -> float:
        ...
        text = text.strip()
        if len(text) == 0:
            print("Пустой текст, снижаем оценку!")
            return -1.0 
    ...
    def train(self, num_episodes: int = 1000, batch_size: int = 4,
        ...
        for episode in range(1, num_episodes + 1):
            ...
            if episode % val_freq == 0:
                val_reward = self._run_validation(episode)
                if self._check_early_stopping(val_reward, patience):
    ...
    def _load_dataset(self, split: str):
        """Load dataset from processed_dataset directory"""
        try:
    ...
    def _run_validation(self, episode: int) -> float:
        ...
        with torch.no_grad():
            if self.val_dataset is None or len(self.val_dataset) == 0:
                print("Валидационный датасет недоступен!")
                return -1.0
                
            # Используем первые несколько примеров для валидации
        ...
        if avg_reward > self.best_reward:
            ...
            self.patience_counter = 0
            print(f"Новая лучшая модель! Вознаграждение на валидации: {avg_reward:.4f}")
        else:
            self.patience_counter += 1
            
        return avg_reward

    def _check_early_stopping(self, val_reward: float, patience: int) -> bool:
...
if __name__ == "__main__":
    ...
    if trainer.train_dataset is None and trainer.val_dataset is None:
        print("Ошибка: train/validation датасеты не загружены. Запусти preprocess/load_dataset.py")
        sys.exit(1)
    
    test_samples = [trainer.val_dataset[i] for i in range(min(3, len(trainer.val_dataset)))]
    ...
    print("Старт обучения REINFORCE...")
    trainer.train(num_episodes=100, batch_size=4, patience=20, val_freq=10)
